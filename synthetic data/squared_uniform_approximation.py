# -*- coding: utf-8 -*-
"""squared uniform approximation.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1TK_83hY2wpeS2ryfmXXcYF4_YEASHgjT
"""
...
def plot_fn_heatmap(Worig,bias):
...
def apply_square(X,W,b,noise):
    Y = np.square(np.matmul(W,X.T)+b)
    return Y + noise  
...
def get_error(Worig,borig,W_hat,b_hat):
    ...
    f_orig = apply_square(X,Worig,borig,0)
    f_hat = apply_square(X,W_hat,b_hat,0)
...
# def latex_plot_error(sample_sizes,error_l,error_u,Worig,borig):
...
#     plt.plot(sample_sizes,error_l,'--',label="Leverage Score Measure",color="green")
#     plt.plot(sample_sizes,error_u,':',label="Uniform measure",color="blue")
...
def fit_relu(sample_sizes,Worig,borig,noise_variance,biases,d=2,num_iterations=100):
    ...
    ones_column = np.ones(grid.shape[0])
    grid = np.insert(grid,2,ones_column,axis=1)
    lev_scores = get_leverage_scores(grid)
    ...
    for m in sample_sizes:
        ...
        for itr in range(num_iterations):
            ...
            Xu = grid[np.random.choice(grid.shape[0], size=m, replace=False),:-1]
            Yu = apply_square(Xu,Worig,borig,noise)
            #Sample data using leverage scores
        
            #We first calculate the leverage scores of the grid
            lev_scores = get_leverage_scores(grid)
            indices = np.random.choice(grid.shape[0],size=m,replace=False,p=lev_scores/np.sum(lev_scores))
            Xl = grid[indices,:-1]
            sampled_lev_scores = lev_scores[indices]
            Yl = apply_square(Xl,Worig,borig,noise)
            ...
            Wmin_u,bmin_u = get_minimiser(Xu,Yu,grid[:,:-1],biases,isleverage=False,lev_scores=sampled_lev_scores)
            W_it_u[itr,:]= Wmin_u
            b_it_u[itr] = bmin_u
            
            #Solving using leverage score sampling
            Wmin_l,bmin_l = get_minimiser(Xl,Yl,grid[:,:-1],biases,isleverage=True,lev_scores=sampled_lev_scores)
            W_it_l[itr,:] = Wmin_l
            b_it_l[itr] = bmin_l
...
Worig = np.array([0.1,0.1])
borig = -0.1

setofW = get_grid(101,101,-1,1)
biases = np.linspace(-0.8,0.8,50)

# We now visualize the function we generated
plot_fn_heatmap(Worig,borig)
sample_sizes = [40,60,75,95,110]

#We try to fit relu using uniform sampling and leverage score sampling
W_hat_uniform,W_hat_leverage,b_hat_uniform,b_hat_leverage = fit_relu(sample_sizes,Worig,borig,noise_variance = 0.2,d=2,biases = biases, num_iterations=100)
...
path = "drive/MyDrive/ICLR_2023/square_uniform/aistats"
# os.mkdir(path)
# !cp error_l_med.npy /content/drive/MyDrive/ICLR_2023/square_uniform/exp_5_var_0.05/
# !cp error_l_mean.npy /content/drive/MyDrive/ICLR_2023/square_uniform/exp_5_var_0.05/
# !cp error_u_med.npy /content/drive/MyDrive/ICLR_2023/square_uniform/exp_5_var_0.05/
# !cp error_u_mean.npy /content/drive/MyDrive/ICLR_2023/square_uniform/exp_5_var_0.05/

!cp orig.png /content/drive/MyDrive/ICLR_2023/square_uniform/aistats/
!cp Error_mean.png /content/drive/MyDrive/ICLR_2023/square_uniform/aistats/
!cp Error_median.png /content/drive/MyDrive/ICLR_2023/square_uniform/aistats/
# !cp sample_sizes.npy /content/drive/MyDrive/ICLR_2023/square_uniform/exp_5_var_0.05/
...
def latex_plot_error(sample_sizes,error_l,error_u,Worig,borig,style,name):
    ...
    plt.plot(sample_sizes,error_u,':',label="Uniform measure")
    plt.legend(loc='upper right', shadow=True)
    tikzplotlib.save(name +'.tex')

plt.rcParams.update(plt.rcParamsDefault)
style = 'seaborn-dark-palette'
latex_plot_error(sample_sizes[2:],error_l_med[2:],error_u_med[2:],Worig,borig,style,'med_plot')
# path = "drive/MyDrive/ICLR_2023/square_uniform/workshop_1"

# os.mkdir(path)
# !cp med_plot.tex /content/drive/MyDrive/ICLR_2023/square_uniform/workshop_1/
